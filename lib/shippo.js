...
export function shipFromAddress() {
  return {
    name: "ERENDIRA'S BOUTIQUE LLC",
    company: "ERENDIRA'S BOUTIQUE LLC",
...
function cleanAddress(a = {}) {
  return Object.fromEntries(
    Object.entries(a).filter(([, v]) => v !== undefined && v !== null && v !== '')
  );
}
...
export function buildReturnShipmentPayload(r = {}) {
  return {
    address_from: cleanAddress({
      ...
      phone: r.customer_phone || r.phone || undefined,
      email: r.customer_email || r.email || undefined
    }),
...
export function cheapestUspsRate(rates = []) {
  return normalizeShippoRates(rates)
    .filter((r) => String(r.carrier || '').toUpperCase() === 'USPS')[0];
}
...
export function normalizeShippoTransaction(tx = {}) {
  ...
  return {
    ...
    status: tx.status === 'SUCCESS' || tx.object_status === 'SUCCESS'
      ? 'label_purchased'
      : (tx.status || tx.object_status || 'label_purchased'),
    batch_selected: true
  };
}
...
export function normalizeShippoReturnTransaction(tx = {}, rate = {}) {
  ...
  return {
    ...
    status: tx.status === 'SUCCESS' || tx.object_status === 'SUCCESS'
      ...
      : (tx.status || tx.object_status || 'return_label_created')
  };
}
