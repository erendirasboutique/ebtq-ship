...
export function buildShipmentPayload(order = {}) {
  const weight =
    order.parcel_weight ||
    toOz(order.parcel_weight_lb, order.parcel_weight_oz) ||
    16;
  ...
  return {
    shipment: {
      ...
      options: {
        ...
        delivery_confirmation: order.signature_required ? 'SIGNATURE' : undefined
      },
...
export function createEasyPostShipment(order) {
...
export function buyEasyPostLabel(shipmentId, rateId) {
...
export function refundEasyPostShipment(shipmentId) {
