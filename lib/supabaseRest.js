...
export async function deleteOrder(id) {
  await sfetch(`/shipping_orders?id=eq.${id}`, {
    method: 'DELETE',
    headers: {
...
export async function mergeCustomers({ primaryId, duplicateIds }) {
  if (!primaryId || !duplicateIds?.length) {
    throw new Error('Missing merge data');
  }

  const ids = duplicateIds.join(',');

  await sfetch(`/shipping_orders?customer_id=in.(${ids})`, {
    method: 'PATCH',
    body: JSON.stringify({ customer_id: primaryId })
  });

  await sfetch(`/shipping_customers?id=in.(${ids})`, {
    method: 'PATCH',
    body: JSON.stringify({
      archived: true,
